const { Client, EmbedBuilder, PermissionsBitField, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, } = require("discord.js"); const db = require("croxydb");

function initialize(client) { client.on("guildMemberAdd", async (member) => { const registrationSystem = db.get(registrationSystem_${member.guild.id}); if (!registrationSystem) return;
...
client.on("interactionCreate", async (interaction) => { if (!interaction.guild || !interaction.customId) return;
...
if (interaction.isButton() && interaction.customId.includes("_register_")) {
  ...
  if (action === "normal") {
    if (interaction.user.id !== targetUserId) {
      return interaction.reply({ content: "❌ | Bu butonu sadece kaydı yapılacak kişi kullanabilir!", flags: 1 << 6 });
    }
  ...
  if (action === "manual") {
    if (!interaction.member.roles.cache.has(staffRoleId) && !interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
      return interaction.reply({ content: "❌ | Bu butonu yalnızca yetkililer kullanabilir.", flags: 1 << 6 });
    }

    const targetMember = await interaction.guild.members.fetch(targetUserId).catch(() => null);
    if (!targetMember) return interaction.reply({ content: "Kullanıcı sunucudan ayrılmış gibi görünüyor.", flags: 1 << 6 });
...
async function processRegistration(interaction, targetMember, newNickname, registrar, config, isNormal) { try { await targetMember.setNickname(newNickname); await targetMember.roles.add(config.registeredRoleId); await targetMember.roles.remove(config.unregisteredRoleId);
...
} catch (error) { console.error("Kayıt işleme hatası:", error); await interaction.editReply({ content: "❌ | Kayıt sırasında bir hata oluştu! Botun rolünün, yönettiği rollerden üstte olduğundan ve yetkilerinin tam olduğundan emin ol." }); } }

module.exports = { commands: [ { name: "registration-system", description: "Gelişmiş kayıt sistemini kurar.", type: 1, options: [ { name: "registration-channel", description: "Kayıt mesajlarının gönderileceği kanal.", type: ChannelType.GuildText, required: true }, { name: "unregistered-role", description: "Yeni üyelere verilecek kayıtsız rolü.", type: 8, required: true }, { name: "registered-role", description: "Kayıt olunca verilecek rol.", type: 8, required: true }, { name: "staff-role", description: "Kayıt yapmaya yetkili rol.", type: 8, required: true }, { name: "tag", description: "İsimlerin başına eklenecek tag.", type: 3, required: true }, { name: "log-kanali", description: "Kayıt loglarının gönderileceği kanal.", type: ChannelType.GuildText, required: true }, { name: "duyuru-kanali", description: "Üye kaydolunca duyurunun yapılacağı sohbet kanalı. (İsteğe Bağlı)", type: ChannelType.GuildText, required: false }, ], run: async (client, interaction) => { if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) { return interaction.reply({ content: "❌ | Bu komutu kullanmak için Yönetici yetkisine ihtiyacınız var.", flags: 1 << 6 }); }
...
},
{
  ...
  run: async (client, interaction) => {
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
      return interaction.reply({ content: "❌ | **Yönetici** iznine ihtiyacınız var.", flags: 1 << 6 });
    }
